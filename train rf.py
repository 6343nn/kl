...
print("-------------------------------------------------------------")
print("--- Script Kết hợp Annotations và Phân loại BI-RADS v5 (RandomForest) ---")
print("-------------------------------------------------------------")
...
test_dir_name = "test"
IMAGE_EXTENSION = ".png"
output_dir = "classification_output_rf_lime_shap"
os.makedirs(output_dir, exist_ok=True)
output_processed_csv_path = os.path.join(output_dir, "final_processed_annotations_rf.csv")
output_model_path = os.path.join(output_dir, "rf_birads_model.pkl")
output_report_path = os.path.join(output_dir, "classification_results_rf.txt")
output_cm_plot_path = os.path.join(output_dir, "confusion_matrix_heatmap_rf.png")
output_roc_plot_path = os.path.join(output_dir, "roc_curves_ovr_rf.png")
output_pr_plot_path = os.path.join(output_dir, "precision_recall_curves_ovr_rf.png")
...
# =============================================================
augmented_images_dir = "augmented_images_rf"
...
# =============================================================
print("\n[Bước 7/10] Huấn luyện mô hình RandomForest và Lưu mô hình...")
start_step_time = time.time()
...
try:
    ...
    print(f"❌ Lỗi khi huấn luyện RandomForest: {e}")
    exit()
end_step_time = time.time()
print(f"✅ Huấn luyện RandomForest hoàn tất (Thời gian: {end_step_time - start_step_time:.2f}s).")
print(f"   - Lưu mô hình RandomForest vào: {output_model_path}")
try:
...
try:
    # Khởi tạo explainer với feature_perturbation="tree_path_dependent" và model_output="raw"
    ...
    plt.title(f"SHAP Summary Plot for RandomForest Model ({len(X_test)} Test Samples)", fontsize=14, pad=20)
    ...
    # Chọn ngẫu nhiên 5 chỉ số nhỏ hơn 2050
    target_num_samples = 5
    valid_range = np.arange(2050)  # Phạm vi từ 0 đến 2049
    ...
    for i, sample_idx in enumerate(selected_indices):
        ...
        pred_class = y_pred[sample_idx] if y_pred is not None else np.argmax(rf_model.predict_proba(X_test[sample_idx:sample_idx+1])[0])
        
        # Tạo SHAP Explanation cho lớp dự đoán
        shap_explanation = shap.Explanation(
            values=shap_values[pred_class][sample_idx] if len(shap_values) > 1 else shap_values[0][sample_idx],
            base_values=explainer.expected_value[pred_class] if len(shap_values) > 1 else explainer.expected_value,
...
try:
    ...
    with open(output_report_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"  - Tham số RandomForest: {json.dumps(RF_PARAMS, indent=4)}\n")
