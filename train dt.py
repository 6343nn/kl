...
print("-------------------------------------------------------------")
print("--- Script Kết hợp Annotations và Phân loại BI-RADS v5 (Gaussian Naive Bayes) ---")
print("-------------------------------------------------------------")
...
test_dir_name = "test"
IMAGE_EXTENSION = ".png"
output_dir = "classification_output_nb_lime_shap"
os.makedirs(output_dir, exist_ok=True)
output_processed_csv_path = os.path.join(output_dir, "final_processed_annotations_nb.csv")
output_model_path = os.path.join(output_dir, "nb_birads_model.pkl")
output_report_path = os.path.join(output_dir, "classification_results_nb.txt")
output_cm_plot_path = os.path.join(output_dir, "confusion_matrix_heatmap_nb.png")
output_roc_plot_path = os.path.join(output_dir, "roc_curves_ovr_nb.png")
output_pr_plot_path = os.path.join(output_dir, "precision_recall_curves_ovr_nb.png")
...
IMAGE_SIZE = (224, 224)
NB_PARAMS = {
    'var_smoothing': 1e-9  # Giá trị mặc định cho Gaussian Naive Bayes
}
TEST_SIZE = 0.2
RANDOM_STATE = 42
...
# =============================================================
# ===             5. TRÍCH XUẤT ĐẶC TRƯNG ativity ===
# =============================================================
augmented_images_dir = "augmented_images_nb"
...
# =============================================================
print("\n[Bước 7/10] Huấn luyện mô hình Gaussian Naive Bayes và Lưu mô hình...")
start_step_time = time.time()
...
try:
    ...
    print(f"❌ Lỗi khi huấn luyện Gaussian Naive Bayes: {e}")
    exit()
end_step_time = time.time()
print(f"✅ Huấn luyện Gaussian Naive Bayes hoàn tất (Thời gian: {end_step_time - start_step_time:.2f}s).")
print(f"   - Lưu mô hình Gaussian Naive Bayes vào: {output_model_path}")
try:
    joblib.dump(nb_model, output_model_path)
...
try:
    print("   - Khởi tạo KernelExplainer...")
    ...
    explainer = shap.KernelExplainer(nb_model.predict_proba, X_background)
    ...
    sample_idx = 0
    model_output = nb_model.predict_proba(X_test[sample_idx:sample_idx+1])[0]
    shap_sum = np.sum(shap_values, axis=1)[sample_idx]
    print(f"   - Debug (mẫu 0): Model output = {model_output}, SHAP values sum = {shap_sum}")
    ...
    plt.title(f"SHAP Summary Plot for Gaussian Naive Bayes Model ({len(X_test)} Test Samples)", fontsize=14, pad=20)
    plt.xlabel("mean(|SHAP value|) [average impact on model output magnitude]", fontsize=12)
    plt.ylabel("Features", fontsize=12)
    plt.tight_layout()
    shap_plot_path = os.path.join(output_dir, "shap_summary_model_nb.png")
    ...
    for i, sample_idx in enumerate(selected_indices):
        ...
        pred_class = y_pred[sample_idx] if y_pred is not None else np.argmax(nb_model.predict_proba(X_test[sample_idx:sample_idx+1])[0])
        ...
        plt.tight_layout()
        waterfall_plot_path = os.path.join(output_dir, f"shap_waterfall_sample{i}_nb.png")
    ...
    print("   - Bỏ qua SHAP do lỗi. Vui lòng kiểm tra dữ liệu hoặc báo cáo vấn đề trên GitHub SHAP.")
end_step_time = time.time()
...
try:
    ...
    with open(output_report_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"  - Tham số Gaussian Naive Bayes: {json.dumps(NB_PARAMS, indent=4)}\n")
