...
print("-------------------------------------------------------------")
print("--- Script Kết hợp Annotations và Phân loại BI-RADS v5 (CNN with Feature Extraction) ---")
print("-------------------------------------------------------------")
...
test_dir_name = "test"
IMAGE_EXTENSION = ".png"
output_dir = "classification_output_cnn_features_lime_shap"
os.makedirs(output_dir, exist_ok=True)
output_processed_csv_path = os.path.join(output_dir, "final_processed_annotations_cnn.csv")
output_model_path = os.path.join(output_dir, "cnn_birads_model.pth")
output_report_path = os.path.join(output_dir, "classification_results_cnn.txt")
output_cm_plot_path = os.path.join(output_dir, "confusion_matrix_heatmap_cnn.png")
output_roc_plot_path = os.path.join(output_dir, "roc_curves_ovr_cnn.png")
output_pr_plot_path = os.path.join(output_dir, "precision_recall_curves_ovr_cnn.png")
...
if dropped_rows > 0:
    print(f"   - Đã loại bỏ {dropped_rows} dòng chứa giá trị thiếu trong các cột ['breast_birads', 'image_id'].")
...
# =============================================================
augmented_images_dir = "augmented_images_cnn"
...
# =============================================================
# ===          7. THIẾT LẬP VÀ HUẤN LUYỆN MÔ HÌNH CNN       ===
# =============================================================
print("\n[Bước 7/10] Thiết lập và huấn luyện mô hình CNN...")
start_step_time = time.time()
...
try:
    input_dim = FEATURE_VECTOR_SIZE + len(METADATA_COLS_FOR_MODEL)
    model = FeatureCNN(input_dim=input_dim, num_classes=num_classes).to(device)
    ...
    print(f"   - Khởi tạo mô hình CNN với input_dim={input_dim}, num_classes={num_classes}")
except Exception as e:
    print(f"❌ Lỗi khi thiết lập CNN: {e}")
    exit()
...
def train_model(model, X_train, y_train, criterion, optimizer, batch_size, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for i in tqdm(range(0, dataset_size, batch_size), desc=f"Epoch {epoch+1}/{num_epochs}"):
            batch_indices = indices[i:min(i + batch_size, dataset_size)]
            inputs = torch.tensor(X_train[batch_indices], dtype=torch.float32).to(device)
            labels = torch.tensor(y_train[batch_indices], dtype=torch.long).to(device)
            optimizer.zero_grad()
        ...
        print(f"   - Epoch {epoch+1}/{num_epochs}, Loss: {running_loss/(dataset_size/batch_size):.4f}")

try:
    train_model(model, X_train, y_train, criterion, optimizer, CNN_PARAMS['batch_size'], CNN_PARAMS['num_epochs'])
    print("   - Huấn luyện CNN thành công.")
except Exception as e:
    print(f"❌ Lỗi khi huấn luyện CNN: {e}")
    exit()
end_step_time = time.time()
print(f"✅ Huấn luyện CNN hoàn tất (Thời gian: {end_step_time - start_step_time:.2f}s).")
print(f"   - Lưu mô hình CNN vào: {output_model_path}")
try:
...
def evaluate_model(model, X_test, y_test, batch_size):
    ...
    with torch.no_grad():
        for i in range(0, len(X_test), batch_size):
            inputs = torch.tensor(X_test[i:i+batch_size], dtype=torch.float32).to(device)
            outputs = model(inputs)
            _, predicted = torch.max(outputs, 1)
            probs = torch.nn.functional.softmax(outputs, dim=1)
            y_true.extend(y_test[i:i+batch_size])
            y_pred.extend(predicted.cpu().numpy())
            y_pred_proba.extend(probs.cpu().numpy())
    return np.array(y_true), np.array(y_pred), np.array(y_pred_proba)

y_test_pred, y_pred, y_pred_proba = None, None, None
metrics_calculated = False
try:
    y_test_pred, y_pred, y_pred_proba = evaluate_model(model, X_test, y_test, CNN_PARAMS['batch_size'])
    if y_pred_proba is not None and y_pred_proba.shape[0] == len(y_test) and y_pred_proba.shape[1] == num_classes:
...
if metrics_calculated:
    try:
        ...
        report_str = classification_report(y_test_pred, y_pred, target_names=target_names, zero_division=0)
        cm = confusion_matrix(y_test_pred, y_pred, labels=range(num_classes))
...
if plot_evaluation_charts:
    ...
    fig_roc = None
    try:
        y_test_binarized = label_binarize(y_test_pred, classes=range(num_classes))
...
try:
    ...
    shap_values = explainer.shap_values(X_test[:5], nsamples=100)
    ...
    plt.title("SHAP Summary Plot for CNN Model", fontsize=14, pad=20)
    plt.tight_layout()
    shap_plot_path = os.path.join(output_dir, "shap_summary_model.png")
    plt.savefig(shap_plot_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"     💾 Lưu biểu đồ SHAP tóm tắt: {shap_plot_path}")
    for i in range(min(5, len(X_test))):
        shap_explanation = shap.Explanation(
            values=shap_values[y_pred[i]][i] if len(shap_values) > 1 else shap_values[i],
            base_values=explainer.expected_value[y_pred[i]] if len(shap_values) > 1 else explainer.expected_value,
            data=X_test[i],
            feature_names=[f"img_f{j}" for j in range(FEATURE_VECTOR_SIZE)] + ['view_position', 'breast_density', 'laterality']
        )
        plt.figure(figsize=(12, 6))
        shap.plots.waterfall(shap_explanation, max_display=10, show=False)
        plt.title(f"SHAP Waterfall Plot - Sample {i} - True: {target_names[y_test[i]]}, Predicted: {target_names[y_pred[i]]}")
        plt.tight_layout()
...
try:
    ...
    with open(output_report_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"  - Model Feature Extraction: {MODEL_NAME}\n")
        f.write(f"  - Kích thước đặc trưng: {FEATURE_VECTOR_SIZE}\n")
        f.write(f"  - Tham số CNN: {json.dumps(CNN_PARAMS, indent=4)}\n")
        ...
        if metrics_calculated:
            ...
            f.write("\nGiải thích SHAP:\n")
            if shap_plot_path:
                f.write(f"  - Biểu đồ SHAP tóm tắt: {shap_plot_path}\n")
            else:
